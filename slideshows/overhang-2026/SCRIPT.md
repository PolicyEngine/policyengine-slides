...
## 8 · Which statistic would this bill move most? (1:05)

Words: 78 · Clock: 5:50 to 6:55

[click] Before any number, the measure. How would you tell whether this bill did its job? Name an official statistic, and say how far the bill would move it in standard deviations of that statistic's own forecast. A useful one moves by more than its own interval. Sixty seconds. [pause for shout-outs; repeat each one back] Returns claiming the credit. Child poverty. Filing among the lowest earners. Keep your list. Layer four.
## 9 · PolicyEngine computes what the change does to each household (1:05)

Words: 150 · Clock: 6:55 to 8:00

[click] PolicyEngine runs that parameter change over a calibrated population of US households and produces the standard outputs. I ran it this afternoon on PolicyEngine's API with the bill's one-dollar threshold: federal cost, 1.83 billion dollars in 2026. Now ask what cost is. It's the gap between two forecasts of the deficit, with and without the bill. Our agent's forecast for one month of the federal deficit, May, carried an 80 percent interval from 240 to 380 billion dollars, and the print came in at 293. Against that band, 1.83 billion is noise. The deficit will never score this bill. This is the kind of number I built PolicyEngine to produce, and the kind CBO and the rest of the scoring world run on: one outcome per run, behavior fixed, no interval attached. So the model hands the forecast a mechanism, and the statistic that scores the bill stays an open choice. An agent made that choice.
## 10 · The agent picked returns claiming the credit (2:05, of which 90 seconds silent)

Words: 77 · Clock: 8:00 to 10:05 · "Go" lands at about 0:31 after the click

[click] The agent picked returns claiming the refundable credit. Your number first: how many tax returns will claim it for tax year 2027 if this bill passes? The IRS first prints: 19.1 million for 2020, 37.8 for 2021, the year the law dropped the earnings test and raised the credit, then 18.1 and 17.6. Threshold, 2,500 dollars to one dollar. Write a median and an 80 percent interval. Ninety seconds, no talking. Go.
...
Words: 214 · Clock: 10:05 to 11:50
...
Words: 118 · Clock: 13:35 to 14:25
...
## 15 · Every layer of the stack exists today, each with its own check (1:00)

Words: 139 · Clock: 14:25 to 15:25 · Objections 15:25 to 27:00

[click] Back to the five things from slide 5, with names on them. The Axiom Foundation encodes the rules and compares their outputs. Chronicle records official statistics with their sources. Microcosm builds the populations and publishes the calibration error. PolicyEngine computes the reforms. Thesis, still a prototype, forecasts the outcomes and scores them as they resolve. The scoreboard on that last layer ranks the other four. Now break it. Three places to start: Goodhart, when the measure becomes the target; who sets the objectives, since an AI proposed the goals on that bill page and the page has countersign and strike buttons; and legitimacy, who gets to contest the model, the forecast, and the use somebody makes of them. Chatham House rules, so nothing gets attributed. One request for the end: tell me which bill should get a forecast next. Who's first?
...
Max: put the indicator question before the model, so the story runs "how would you measure this bill, in standard deviations of the statistic's own forecast?" then "PolicyEngine does the standard ones; cost is the gap between two deficit forecasts and a tiny fraction of the deficit band" then "the agent picked returns claiming the credit." Child poverty card dropped. The conductor slide no longer cites slide numbers. "Where does this break?" is gone; the objections open from the five-layer slide. The PolicyEngine app toggle came out: policyengine.org redirects to legacy.www.policyengine.org, which did not answer at 6:20 PM, so the frame stayed blank. Talk ends near 15:25.
