...
export default function PrimitivesSlide() {
  ...
  return <div className="relative">
    {roadmap ? <Slide><SlideHeader><SlideTitle>Give each layer a feedback loop</SlideTitle></SlideHeader><div className="mt-10 grid grid-cols-5 gap-4">{layers.map(layer=><div key={layer.name} className="content-card p-5"><div className="flex h-20 items-center"><BasePathImage src={layer.logo} alt={layer.name} width={layer.width} height={layer.height} className="h-auto max-h-14 w-full object-contain" /></div><p className="mt-5 text-lg leading-relaxed text-gray-600">{layer.detail}</p>{layer.name.includes('prototype') && <p className="mt-4 text-sm font-semibold text-pe-teal">Prototype</p>}</div>)}</div></Slide> : <FivePrimitivesSlide feedbackLoops={[
