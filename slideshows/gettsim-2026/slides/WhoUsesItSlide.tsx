...
export default function WhoUsesItSlide() {
  return (
    <Slide>
      <SlideHeader>
        <SlideTitle>
          Researchers, governments, and builders use these rules
        </SlideTitle>
      </SlideHeader>

      <div className="grid grid-cols-3 gap-4 mt-5">
        {evidence.map((item) => (
          <div key={item.label} className="content-card p-4">
      ...
      <div className="w-full mt-6">
        <div className="grid grid-cols-6 gap-x-7 gap-y-6 w-full px-4">
          {organizations.map((org) => (
            ...
            >
              <Image
                ...
                className={`object-contain max-h-12 ${org.className ?? ''}`}
              />
            </div>
          ))}
        </div>
