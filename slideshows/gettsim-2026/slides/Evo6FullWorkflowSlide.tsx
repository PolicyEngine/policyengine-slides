      ...
      <div className="flex gap-6 h-[calc(100vh-280px)]">
        ...
        <div className="w-[65%] bg-[#f9f8f6] rounded-xl p-2 border border-gray-100 flex items-center justify-center">
          <svg className="full-width-flow-svg" viewBox="0 0 950 550" style={{ maxWidth: 850 }}>
            ...
            <text x="430" y="440" textAnchor="middle" fontFamily="JetBrains Mono" fontSize="10" fill="var(--text-secondary)" fontStyle="italic">iterate until tests pass</text>
              ...
              skill.agents.map((agentId) => {
                ...
                return (
                  <line key={`${skill.id}-${agentId}`} x1={skill.x} y1={skillY} x2={agent.x} y2={agent.y + agent.r}
                    stroke="var(--pe-teal)" strokeWidth={isHighlighted ? 2 : 1}
                    strokeDasharray={isHighlighted ? "none" : "4 3"}
                    opacity={hoveredSkill === null ? 0.6 : isHighlighted ? 1 : 0.15}
            ...
            <text x="55" y={skillY + skillHeight / 2 + 4} fontFamily="JetBrains Mono" fontSize="10" fill="var(--text-secondary)" fontWeight="600">Skills</text>
            {skills.map((skill) => {
              ...
              return (
                <g key={skill.id} onMouseEnter={() => setHoveredSkill(skill.id)} onMouseLeave={() => setHoveredSkill(null)} style={{ cursor: 'pointer' }}>
                  <rect x={skill.x - skillWidth / 2} y={skillY} width={skillWidth} height={skillHeight} rx="4"
                    fill={isHovered ? "var(--pe-teal)" : "#E6FFFA"} stroke="var(--pe-teal)"
                  ...
                  <text x={skill.x} y={skillY + skillHeight / 2 + 4} textAnchor="middle" fontFamily="JetBrains Mono" fontSize="10"
                    fill={isHovered ? "white" : "var(--pe-teal)"} fontWeight={isHovered ? 600 : 400}
                    style={{ transition: 'all 0.2s ease', pointerEvents: 'none' }}>{skill.label}</text>
                </g>
              );
            })}
            <text x="475" y="540" textAnchor="middle" fontFamily="JetBrains Mono" fontSize="10" fill="var(--text-secondary)" fontStyle="italic">Hover over a skill to see which agents use it</text>
          </svg>
        </div>
